...
// Computed
const showSplash = computed(() => !weatherStore.hasData && weatherStore.isLoading)
const weather = computed(() => weatherStore.interpolatedWeather || weatherStore.current)
const daily = computed(() => weatherStore.daily[0])

const mainContentRef = ref<HTMLElement | null>(null)

// Watch for city changes to trigger transition
watch(() => locationStore.currentCity, () => {
  if (mainContentRef.value && !settingsStore.prefersReducedMotion) {
    gsap.fromTo(mainContentRef.value, 
    ...
    gsap.set(mainContentRef.value, { opacity: 1, scale: 1, filter: 'blur(0px)' })
  }
})
  ...
  <div class="app min-h-screen relative overflow-hidden">
    ...
    >
      <div class="max-w-[1400px] mx-auto">
        <!-- Search Section -->
        <div v-if="!weatherStore.hasData" class="flex flex-col items-center justify-center h-[70vh] text-center">
          <h1 class="font-display text-7xl md:text-9xl font-black tracking-tighter mb-8 animate-fade-in">
            Weather<span class="text-outline">Vue</span>
          </h1>
          <CitySearch />
          <p class="mt-8 text-xs uppercase tracking-[0.4em] text-muted animate-pulse-slow">
        ...
        <template v-else>
          ...
          <!-- Bento Grid Layout -->
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 mb-12">
            <!-- Temperature Graph (Large) -->
            <div class="lg:col-span-8 bg-glass p-8 rounded-3xl border border-default shadow-glass">
              <TemperatureGraph />
            </div>

            <!-- Wind (Medium) -->
            <div class="lg:col-span-4 bg-glass p-8 rounded-3xl border border-default shadow-glass flex items-center justify-center">
              <WindCompass 
                v-if="weather"
            ...
            <div v-if="daily" class="lg:col-span-4 bg-glass p-8 rounded-3xl border border-default shadow-glass">
              <SunriseSunsetArc 
            ...
            <!-- UV Index (Small) -->
            <div class="lg:col-span-4 bg-glass p-8 rounded-3xl border border-default shadow-glass">
              <UVIndexGauge 
                v-if="weather"
            ...
            <div class="lg:col-span-4 bg-glass p-8 rounded-3xl border border-default shadow-glass">
              <PrecipitationBar 
                v-if="weather"
                :probability="'precipitationProbability' in weather ? (weather.precipitationProbability ?? 0) : 0"
